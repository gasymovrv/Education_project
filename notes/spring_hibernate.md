...
+ Подробнее о Spring Boot [тут](https://habr.com/ru/company/jugru/blog/424503/)


## Spring Data
Состоит из transactionManager, dataSource и entityManagerFactory. 
Можно подключить различные реализации стандарта JPA, один из самых популярных - это Hibernate ORM
...
#### Isolation
+ `Isolation.READ_UNCOMMITTED` — читает неподтвержденные данные, самый быстрый, небезопасный уровень, возможны все проблемы
...
Подробнее о проблемах [тут](databases.md)
...
return createNewTransaction();
```
+ Подробнее [тут](https://www.baeldung.com/spring-transactional-propagation-isolation)
...
+ Вызов метода с `@Transactional` в другом методе в том же бине
