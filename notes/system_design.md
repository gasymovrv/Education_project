...
+ Что делает система? → Определи тип нагрузки: транзакционная (booking, оплата), контентная (видео, медиа), коммуникационная (чаты, уведомления).
+ Какие основные сценарии будем прорабатывать? → Например, 2 сценария для видео хостинга - загрузка видео контент мэйкером и просмотр юзером
+ Остальное зависит от задачи

---

## Определяем границы проектирования
**1. Назначение системы**  
Кратко сформулировать, какую задачу решает система и какую основную функцию выполняет.  
_Пример: система хранит и раздаёт пользовательские видео; не отвечает за обработку или рекомендации._

**2. Входит в проектирование (In scope)**
...
**3. Не входит в проектирование (Out of scope)** 
...
Указать все внешние системы, с которыми требуется взаимодействовать. 
...
## Прикидочный расчет мощностей на примерах
